...
const NavBar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-black bg-opacity-20 text-white py-4 px-6 flex justify-between items-center z-50">
      {/* Left Icons */}
      ...
      {menuOpen && (
        <div className="absolute inset-0 z-40 bg-black flex flex-col justify-center items-center space-y-6">
          {/* Close Button */}
          <button
          ...
          {menuItems.map((item, index) => (
            <a
              ...
              className="text-4xl font-phudu uppercase tracking-widest transition-transform duration-300 hover:scale-105 text-white"
              onClick={() => setMenuOpen(false)}
            >
              {item}
            </a>
